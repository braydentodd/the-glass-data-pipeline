"""
The Glass Sheets - NCAA Library Module

Reusable utilities for Google Sheets sync and API (NCAA basketball).
All functions operate on config data from config.ncaa_sheets.
Follows same pattern as lib/nba_sheets.py but adapted for NCAA.
"""
...
from config.ncaa_etl import DB_CONFIG, DB_SCHEMA, NCAA_CONFIG
from lib.ncaa_etl import get_table_name
from lib.db import get_db_connection
from config.ncaa_sheets import (
...
class SheetsConfigurationError(Exception):
    """Raised when config/ncaa_sheets.py has invalid formula syntax."""
    pass
...
def fetch_players_for_team(conn, team_abbr: str, section: str = 'current_stats',
    ...
    # Entity fields — no age computation, compute experience from actual seasons
    p_fields_base = [f'p.{_quote_col(f)}' for f in sorted(_PLAYER_ENTITY_FIELDS)
                     if f not in ('updated_at', 'birthdate', 'years_experience')]
    # Compute years_experience as COUNT of distinct regular-season appearances
    exp_subquery = (
    ...
    if section == 'current_stats':
        # Single season — INNER JOIN so only players with current-season stats appear
...
def _build_year_filter(years_config: Optional[dict], current_year: int,
    ...
    Converts to season strings (e.g. '2025-26') to match the varchar season column.
    """
    if not years_config:
    ...
    if mode == 'career':
        if include_current:
            ...
            # Exclude current season — career previous only
...
def calculate_all_percentiles(all_entities: List[dict], entity_type: str,
    ...
    Each stat value is weighted by minutes played for its stat_category:
      ...
      - none     → weight = 1  (unweighted — games, minutes, years, entity info)
    ...
    # Build sorted (value, weight) arrays for each stat column with has_percentile
    percentiles = {}
    for col_key, col_def in SHEETS_COLUMNS.items():
...
def get_percentile_rank(value: Any, sorted_weighted: List, reverse: bool = False) -> float:
    ...
    # Accumulate weight below and at the value
    weight_below = 0.0
    weight_equal = 0.0
    for v, w in sorted_weighted:
        if v < value:
            ...
            weight_equal += w
        # sorted, so once past we can break
        elif v > value:
            break

    # Midpoint of the weight range occupied by this value
...
def build_sheet_columns(entity: str = 'player', stat_mode: str = 'both',
    ...
    #   'teams'  = individual team sheets (DUKE, UNC, …)
    ...
    def _normalize_sheets(col_def):
        ...
        if isinstance(col_sheets, str):
            ...
            elif col_sheets in ('players', 'ncaa'):
                return ['all_players']
...
def _insert_opponent_columns(columns: List[Tuple], pct_columns: dict,
    ...
    for entry in columns:
        ...
        # Detect transition away from defense within same section (e.g. defense → efficiency)
...
def build_entity_row(entity_data: dict, columns_list: List[Tuple],
    ...
    for entry in columns_list:
        ...
        # Years column: show count of distinct years (already COUNT(DISTINCT s.season) from SQL)
        if col_key == 'years':
            ...
            if section_data and is_stats_section and col_ctx in section_data:
                ...
                year_count = None
            # year count is already an integer from COUNT(DISTINCT s.season)
...
def format_section_header(section: str, years_config: Optional[dict] = None,
    ...
    Historical/Post: "Last 3 Regular Season Stats (2023-24 to 2025-26) per 40 Mins"
    ...
    Args:
        ...
        mode: Stats display mode ('per_game', 'per_40', 'per_100', 'totals')
    """
    _MODE_LABELS = {
        'per_game': 'per Game',
        'per_40': 'per 40 Mins',
...
def build_formatting_requests(ws_id: int, columns_list: List[Tuple],
    ...
    if not hide_advanced:
        ...
        sub_hdr_row = fmt['subsection_header_row']
...
def _get_value_at_percentile(sorted_values: List, percentile: float,
    ...
    Accepts either plain numeric values or (value, weight) tuples (as returned
    by calculate_all_percentiles).  Only the value component is used for
    interpolation — weights are ignored here since we just want the value at
    a given position in the distribution.
    """
    if not sorted_values:
        return None

    def _v(entry):
        """Extract numeric value from a plain number or (value, weight) tuple."""
        return entry[0] if isinstance(entry, tuple) else entry

    n = len(sorted_values)
    if n == 1:
        return _v(sorted_values[0])
    if reverse:
        percentile = 100 - percentile
    ...
    frac = idx - lower
    v_lower = _v(sorted_values[lower])
    v_upper = _v(sorted_values[upper])
...
def get_config_for_export(mode: str = 'per_100') -> dict:
    ...
    Exports:
      - column_ranges:            section toggle ranges (team_sheet / ncaa_sheet)
    ...
    """
    from lib.ncaa_etl import get_teams_from_db

    # --- NCAA teams dict --------------------------------------------------
    teams_from_db = get_teams_from_db()
    ncaa_teams = {abbr: team_id for team_id, (abbr, institution) in teams_from_db.items()}
    ...
    ncaa_columns = build_sheet_columns(
        entity='player', stat_mode='both',
    ...
    column_ranges = {'team_sheet': {}, 'ncaa_sheet': {}, 'teams_sheet': {}}
    ...
    advanced_column_ranges = {
        ...
        'ncaa_sheet':  _contiguous_ranges(_advanced_indices(ncaa_columns)),
    ...
    basic_column_ranges = {
        ...
        'ncaa_sheet':  _contiguous_ranges(_basic_indices(ncaa_columns)),
    ...
    percentile_column_ranges = {
        ...
        'ncaa_sheet':  _contiguous_ranges(_percentile_indices(ncaa_columns)),
    ...
    base_value_column_ranges = {
        ...
        'ncaa_sheet':  _contiguous_ranges(_base_value_with_pct_indices(ncaa_columns)),
    ...
    subsection_boundaries = {
        ...
        'ncaa_sheet':  _boundary_entries(ncaa_columns,   _get_subsection_boundaries(ncaa_columns)),
    ...
    section_boundaries = {
        ...
        'ncaa_sheet':  _boundary_entries(ncaa_columns,   _get_section_boundaries(ncaa_columns)),
    ...
    stats_section_ranges = {
        ...
        'ncaa_sheet':  _stats_section_range(ncaa_columns),
    ...
    column_metadata = {
        ...
        'ncaa_sheet':  _column_metadata(ncaa_columns),
    ...
    column_widths = {
        ...
        'ncaa_sheet':  _column_widths(ncaa_columns),
        'teams_sheet': _column_widths(teams_columns),
    }

    # --- Column indices for edit detection (1-indexed for Sheets) --------
    ncaa_id_idx = get_column_index('ncaa_id', team_columns)
    team_col_idx = get_column_index('team', ncaa_columns)
    stats_start = None
    ...
    for col_key, col_def in SHEETS_COLUMNS.items():
        ...
        ncaa_idx = get_column_index(col_key, ncaa_columns)
        editable_columns.append({
            'col_key': col_key,
            'team_col_index': (team_idx or 0) + 1,
            'ncaa_col_index': (ncaa_idx or 0) + 1 if ncaa_idx is not None else None,
            'db_field': db_field,
    ...
    # Reverse mapping: institution name → abbreviation (for Teams sheet edit detection)
    team_name_to_abbr = {institution: abbr for _, (abbr, institution) in teams_from_db.items()}
