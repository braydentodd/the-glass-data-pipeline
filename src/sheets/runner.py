...
from sheets.lib.layout import (
    build_headers,
    build_merged_entity_row,
    build_sheet_columns,
    build_summary_rows,
)
from sheets.lib.calculations import (
    calculate_all_percentiles,
    _eval_dynamic_formula
)
from sheets.lib.formatting import build_formatting_requests
from sheets.lib.google import (
    get_sheets_client,
...
class LeagueSyncContext:
    ...
    # Added properties for db.py execution since we removed nba_lib / ncaa_lib
    @property
    def player_entity_table(self): return self.etl_lib.get_table_name('player_entity')
    @property
    def team_entity_table(self): return self.etl_lib.get_table_name('team_entity')
    @property
    def player_stats_table(self): return self.etl_lib.get_table_name('player_stats')
    @property
    def team_stats_table(self): return self.etl_lib.get_table_name('team_stats')

    @property
    def player_entity_fields(self): return _PLAYER_ENTITY_FIELDS
    @property
    def team_entity_fields(self): return _TEAM_ENTITY_FIELDS

    @property
    def stat_fields(self):
        # We compute this dynamically by taking all columns matching this league,
        # subtracting entity fields.
        from sheets.config.columns import SHEETS_COLUMNS
        name = 'nba' if 'nba' in self.league_config.get('current_season_year', '') or hasattr(self.etl_lib, 'get_table_name') and 'nba' in self.etl_lib.__name__ else 'ncaa'
        if 'ncaa' in self.etl_lib.__name__: name = 'ncaa'
        
        all_cols = {k for k, v in SHEETS_COLUMNS.items() if name in v.get('leagues', []) and v.get('stat_category') != 'none'}
        return {c for c in (all_cols - _PLAYER_ENTITY_FIELDS - _TEAM_ENTITY_FIELDS) if not c[0].isupper()}

    @property
    def team_stat_fields(self):
        from sheets.config.columns import SHEETS_COLUMNS
        name = 'nba' if 'nba' in self.etl_lib.__name__ else 'ncaa'
        all_cols = {k for k, v in SHEETS_COLUMNS.items() if name in v.get('leagues', []) and v.get('stat_category') != 'none'}
        return {c for c in (all_cols - _TEAM_ENTITY_FIELDS) if not c[0].isupper()}

    @property
    def team_abbr_col(self): return self.team_abbr_field
    
    @property
    def primary_minutes_col(self): return 'minutes_x10' if 'minutes_x10' in self.stat_fields else 'minutes'

    @property
    def year_format_fn(self):
        # returns the local formatting func if needed, else None
        if 'nba' in self.etl_lib.__name__:
            return lambda y: f"{y}-{y+1}" # simple stub
        return str
...
def _get_team_names(ctx):
    ...
    teams_db = ctx.etl_get_teams_from_db()
